...
#include "voyage/cotila/detail/tmp.h"
#include <array>
#include <cstddef>
#include <type_traits>
...
template <typename T, std::size_t N> struct vector {
   ...
   *
   *  Returns a reference to the scalar element in position `i`, without bounds
   * checking.
   */
 ...
 *  \code{.cpp}
 *  cotila::vector v{1., 2.}; // deduces the type of v to be
 * cotila::vector<double, 2>
 *  \endcode
 */
template <typename T, typename... U>
vector(T, U...) -> vector<std::enable_if_t<(std::is_same_v<T, U> && ...), T>,
                          1 + sizeof...(U)>;
 ...
 *  This deduction guide allows cotila::vector to be constructed like this:
 *  \code{.cpp}
 *  cotila::vector v{{{1., 0}, {2., 1}}}; // deduces the type of v to be
 * cotila::vector<std::complex<double>, 2>
 *  \endcode
 */
