 ...
 *  @return an N-vector of type T with elements described by \f$
 * f\left(\textbf{v}_i, \ldots\right) \f$
 *
 ...
 *  @param f a function of type F that operates between U and vector elements of
 * type T
 *  @return \f$ f\left(f\left(\ldots f\left(\textrm{init}, \textbf{v}_1\right),
 * \ldots\right), \textbf{v}_N \right) \f$
 *
 *  Accumulates an operation over the elements.  This is equivalent to a
 * functional fold.
 */
 ...
 *  @return an N-vector of type T containing the casted elements of \f$
 * \textbf{v} \f$
 *
 ...
 *  @return an N-vector of type T such that \f$ \textbf{v}_i = \textrm{start} +
 * i - 1 \f$
 *
 ...
 *  @return an N-vector of type T with elements spaced by \f$ \frac{\textbf{v}_N
 * - \textbf{v}_1}{N-1} \f$
 *
 ...
 *  @return an N-vector of type T such that \f$ \textbf{v}_i =
 * \textrm{value}\ \forall i \in 1\ .. N\f$
 *
 ...
 *  @return an N-vector with type matching the return type of f such that \f$
 * \textbf{v}_i = f(i) \f$
 *
 ...
 *  \f$ \left(\textbf{v} \gg n\right)_i = \textbf{v}_{(i + n)\ \textrm{mod}\ N}
 * \f$
 *
 ...
 *  @return an M-vector \f$ \textbf{v}_{\textrm{start}:\left(\textrm{start} + M
 * - 1\right)} \f$ such that  \f$
 * \left(\textbf{v}_{\textrm{start}:\left(\textrm{start} + M -
 * 1\right)}\right)_i = \textbf{v}_{\textrm{start} + i} \f$
 *
 ...
 *  \f$ \left(\left[\textbf{a} \textbf{b}\right]\right)_i = \begin{cases}
 * \textbf{a}_i & i < N\\ \textbf{b}_{i - N} & i \ge N \end{cases} \f$
 *
 *  Slices a vector into a subvector.
 */
