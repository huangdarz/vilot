...
#include "voyage/cotila/detail/assert.h"
 ...
 *  @return an \f$ M \times N \f$ matrix \f$\overline{\textbf{m}}\f$ of type T
 * such that
 *  \f$ \left(\overline{\textbf{m}}\right)_{ij} =
 * \overline{\textbf{m}_{ij}},\ \forall i,j \f$
 *
 ...
 *  @param m an \f$ M \times N \f$ matrix of type T
 *  @return an \f$ N \times M \f$ matrix \f$ \textbf{m}^{\mathrm{T}} \f$ of type
 * T such that
 *  \f$ \left(\textbf{m}^{\mathrm{T}}\right)_{ij} = \textbf{m}_{ji},\ \forall
 * i,j \f$
 *
 ...
 *  @param m an \f$ M \times N \f$ matrix of type T
 *  @return an \f$ N \times M \f$ matrix \f$ \textbf{m}^{*} \f$ of type T such
 * that
 *  \f$ \left(\textbf{m}^{*}\right)_{ij} = \overline{\textbf{m}_{ji}},\ \forall
 * i,j \f$
 *
 ...
 *  @return an \f$ M \times P \f$ matrix \f$ \textbf{a}\textbf{b} \f$ of type T
 * such that
 *  \f$ \left(\textbf{ab}\right)_{ij} =
 * \sum\limits_{k=1}^{N}\textbf{a}_{ik}\textbf{b}_{kj} \f$
 *
...
constexpr matrix<T, M, P> matmul(const matrix<T, M, N> &a,
  ...
  return generate<M, P>(
      [&a, &b](auto i, auto j) { return cotila::sum(a.row(i) * b.column(j)); });
}

/** @brief Computes the kronecker tensor product
 *  @param a an \f$M \times N\f$ matrix
 *  @param b an \f$P \times Q\f$ matrix
 *  @return An \f$ MP \times NQ \f$ matrix \f$ \textbf{a}\otimes\textbf{b} \f$
 * of type T such that
 *  \f$ \left(\textbf{a}\otimes\textbf{b}\right)_{ij} = \textbf{a}_{\lfloor i/P
 * \rfloor,\lfloor j/Q \rfloor}\textbf{b}_{i\textrm{%}P,j\textrm{%}Q} \f$ where
 * \f$ i \textrm{%} P \f$ is the remainder of \f$ i/P \f$
 *
...
constexpr matrix<T, M * P, N * Q> kron(const matrix<T, M, N> &a,
  ...
  return generate<M * P, N * Q>(
      [&a, &b](auto i, auto j) { return a[i / P][j / Q] * b[i % P][j % Q]; });
}
 ...
 * type T such that \f$ {\left\lVert \textbf{m} \right\rVert}_\infty =
 * \max\limits_i
 ...
 */
template <typename T, std::size_t M> constexpr T det(const matrix<T, M, M> &m) {
 ...
 *  @param m an \f$ M \times M \f$ matrix of type T
 *  @return the trace of \f$ \textbf{m} \f$, \f$
 * \textrm{tr}\left(\textbf{m}\right) \f$ such that \f$
