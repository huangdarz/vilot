 ...
 *  @return an \f$ N \times M \f$ matrix of type T with elements described by
 * \f$ f\left(\textbf{m}_{ij}, \ldots\right) \f$
 *
 ...
 *  @return an \f$ N \times M \f$ matrix of type T containing the casted
 * elements of \f$ \textbf{M} \f$
 *
 ...
 *  @return an \f$ N \times M \f$ matrix with type matching the return type of f
 * such that \f$ \textbf{m}_{ij} = f(i, j) \f$
 *
 ...
 *  @return an \f$ N \times M \f$ matrix of type T such that \f$ \textbf{m}_{ij}
 * = \textrm{value}\ \forall i,j \f$
 *
...
constexpr matrix<T, N, N> identity = generate<N, N>(
    [](std::size_t i, std::size_t j) { return T(i == j ? 1 : 0); });
 ...
 *  @return an \f$ \left(M\times\textrm{Row}\right) \times
 * \left(N\times\textrm{Col}\right) \f$ matrix \f$ \textbf{m}' \f$ of type T
 * such that
 * \f$ \textbf{m}'_{ij} =
 * \textbf{m}_{\left(i\ \textrm{mod}\ M\right),\ \left(j\ \textrm{mod}\ N
 * \right)} \f$
 *
 *  Repeats copies of a matrix.
 */
template <std::size_t Row, std::size_t Col, std::size_t M, std::size_t N,
          typename T>
constexpr matrix<T, M * Row, N * Col> repmat(const matrix<T, M, N> &m) {
  return generate<M * Row, N * Col>(
      [&m](std::size_t i, std::size_t j) { return m[i % M][j % N]; });
}
 ...
 *  \f$ {\textbf{m}'}_{ij} = \begin{cases} \textbf{m}_{bj} & i =
 * a\\ \textbf{m}_{aj} & i = b\\ \textbf{m}_{ij} & \textrm{otherwise}
 * \end{cases} \f$
 *
 *  Swap two rows of a matrix.
 */
template <std::size_t M, std::size_t N, typename T>
constexpr matrix<T, M, N> swaprow(matrix<T, M, N> m, std::size_t a,
                                  std::size_t b) {
  for (int i = 0; i < N; i++) {
    T tmp = m[a][i];
    m[a][i] = m[b][i];
 ...
 *  @return an \f$ M \times N \f$ matrix \f$ \textbf{m}' \f$ of type T such that
 *  \f$ {\textbf{m}'}_{ij} = \begin{cases} \textbf{m}_{ib} & j =
 * a\\ \textbf{m}_{ia} & j = b\\ \textbf{m}_{ij} & \textrm{otherwise}
 * \end{cases} \f$
 *
 *  Swap two rows of a matrix.
 */
template <std::size_t M, std::size_t N, typename T>
constexpr matrix<T, M, N> swapcol(matrix<T, M, N> m, std::size_t a,
                                  std::size_t b) {
  for (int i = 0; i < N; i++) {
    T tmp = m[i][a];
    m[i][a] = m[i][b];
 ...
 *  @return an \f$ M \times \left(N+P\right) \f$ matrix of type T \f$
 * \left[\textbf{a} \textbf{b}\right] \f$ such that
 *  \f$ \left(\left[\textbf{a} \textbf{b}\right]\right)_{ij} = \begin{cases}
 * \textbf{a}_{ij} & j < N\\ \textbf{b}_{i,\ \left(j - N\right)} & j \ge N
 * \end{cases} \f$
 *
 *  Horizontally concatenates two matrices.
 */
template <std::size_t M, std::size_t N, std::size_t P, typename T>
constexpr matrix<T, M, N + P> horzcat(const matrix<T, M, N> &a,
 ...
 *  @return an \f$ \left(M+N\right) \times P \f$ matrix of type T \f$
 * \left[\textbf{a}; \textbf{b}\right] \f$ such that
 *  \f$ \left(\left[\textbf{a}; \textbf{b}\right]\right)_{ij} = \begin{cases}
 * \textbf{a}_{ij} & i < M\\ \textbf{b}_{\left(i - M\right),\ j} & i \ge M
 * \end{cases} \f$
 *
 *  Vertically concatenates two matrices.
 */
template <std::size_t M, std::size_t N, std::size_t P, typename T>
constexpr matrix<T, M + N, P> vertcat(const matrix<T, M, P> &a,
 ...
 *  @return an \f$ P \times Q \f$ submatrix \f$ \textbf{m}' \f$ of type T such
 * that
 *  \f$ {\textbf{m}'}_{ij} = \textbf{m}_{\left(a + i\right),\ \left(b +
 * j\right)} \f$
 *
...
template <std::size_t P, std::size_t Q, std::size_t M, std::size_t N,
          typename T>
constexpr matrix<T, P, Q> submat(const matrix<T, M, N> &m, std::size_t a,
                                 std::size_t b) {
  if ((a + P > M) || (b + Q > N))
    throw "index out of range";
  return generate<P, Q>(
      [&m, &a, &b](std::size_t i, std::size_t j) { return m[a + i][b + j]; });
}
 ...
 *
 *  Reshapes a matrix without changing the order of the elements in memory (in
 * row-major order).
 */
template <std::size_t P, std::size_t Q, std::size_t M, std::size_t N,
          typename T>
constexpr matrix<T, P, Q> reshape(const matrix<T, M, N> &m) {
  static_assert(P * Q == M * N,
                "Reshaped matrix must preserve size! P*Q != M*N");
  return generate<P, Q>([&m](std::size_t i, std::size_t j) {
